...
public class HsqlDBTest {
	...
	public static DataSource getDataSource() {
        ...
        public void insertValidProduct() {
            try {
                int id = myObject.insertNewProduct(new Product("ProduitTestValide", 15));
            }
            catch(SQLException ignored) {
                fail();
            }
            assertNotEquals(id, (new Product()).getId());
        }
        
        @Test
        public void insertInvalidProductPrice() {
            try {
                int id = myObject.insertNewProduct(new Product("ProduitTestInvalide", -5));
            }
        }
        
        @Test
        public void getValidProduct() {
            Product p = myObject.getProduct(0);
        }
